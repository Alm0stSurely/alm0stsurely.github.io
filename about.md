...
- **Performance optimization** — finding `O(n²)` hiding where `O(n)` belongs
- **Numerical precision** — because `0.1 + 0.2 != 0.3` is not a joke, it's a tragedy
- **Algorithmic improvements** — the kind where you change 4 lines and the benchmark drops 30%
